...
const TIME_SCALE = 10000;  // Adjusts time to speed up motion
const SCALE_DOWN = 1e9;  // Scale down positions to fit into view
const Y_OFFSET = -5;

const NearEarthObject: React.FC<NearEarthObjectProps> = ({ orbitalElements, name, }) => {
    ...
    useFrame(({ clock }) => {
        ...
        const n = Math.sqrt(GM / Math.pow(orbitalElements.a, 3));
    ...
    const updateOrbitPoints = () => {
        ...
        const orbitalPeriod = 2 * Math.PI * Math.sqrt(Math.pow(orbitalElements.a, 3) / GM);
        ...
        <>
            <group key={name} rotation={[Math.PI / 2, 0, 0]}>
                ...
                <group ref={meshRef} rotation={[Math.PI / -2, 0, 0]}  >
                    ...
                    <PlanetLabel color='#ffffff' onClick={() => { }} text={name} position={[0, 0, 0]} />
                </group>
