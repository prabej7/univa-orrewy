...
const Sun: React.FC<SunProps> = ({ position, onClick, isKepler }) => {
  ...
  // Define the planets' positions, radii, and elliptical parameters
  const planets = [
    {
      name: "Mercury",
      semiMajorAxis: 10,
...
const PlanetOrbit = ({
  ...
  const orbitPoints: [number, number, number][] = Array.from({
    ...
    ] as [number, number, number]; // Explicitly cast to tuple type
  });

  // Use Kepler's second law to dynamically adjust the planet's speed
  useFrame(({ clock }) => {
